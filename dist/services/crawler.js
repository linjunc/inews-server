...
const handleComment = (articleId, itemId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // 写入评论表表，更新文章表
    const currentArticleComment = yield getComment(itemId);
    ...
    for (let i = 0; i < articleComment.length; i++) {
        ...
        // 创建评论
        const commentMo = yield comment_1.default.create({
            create_time: comment.create_time,
            text: comment.text,
            digg_count: comment.digg_count,
            digg_id_list: [],
            user_id: "65e540c2fb1b2651947f9dae",
    ...
    const a = yield article_1.default.updateOne({ _id: articleId }, { $set: { comment_count: articleComment.length } });
});
const crawler = (type) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        console.log("crawler", type);
        const response = yield axios_1.default.get(`https://m.toutiao.com/list/?tag=${type}&count=200&format=json_raw&as=A12665FD9DCA600`);
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const newImageUrls = yield (0, transfer_1.transferImages)({
                urls: transformImageList,
                fileName: "imageList",
            });
            ...
            const newFirstImage = yield (0, transfer_1.transferImage)({
                url: firstImage,
                fileName: `inews-header/${title}/image_0`,
            });
            const articleDb = {
                tag,
                title,
            ...
            // 如果有这个文章，则更新
            if (currentArticle) {
                yield article_1.default.updateOne(articleDb);
                yield handleComment(currentArticle.id, detail.gid);
                continue;
            }
            // 处理完文章信息后，写入文章表
            const newArticle = yield article_1.default.create(articleDb);
            // 处理评论
            yield handleComment(newArticle.id, detail.gid);
        }
    }
    catch (err) {
        console.log(err);
