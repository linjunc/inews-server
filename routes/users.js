module.exports = (app) => {
  ...
  const sToMinute = require("../utils/util_sToMinute");
  ...
  router.get("/history_list", async (req, res) => {
    try {
       ...
       */
      for (let i = skip; i < historyList.length && i < skip + n; i++) {
      ...
      } else if (n + skip >= historyList.length) {
        has_more = false;
      }
  ...
  router.get("/article_list_digg", async (req, res) => {
    try {
       ...
       */
      for (let i = skip; i < userDiggList.length && i < skip + n; i++) {
        const res = await articleModel.findOne(
          { _id: userDiggList[i] },
          projection
        );
      ...
      } else if (n + skip >= userDiggList.length) {
        has_more = false;
      }
  ...
  router.get("/article_list_like", async (req, res) => {
    try {
       ...
       */
      for (let i = skip; i < userLikeList.length && i < skip + n; i++) {
        const res = await articleModel.findOne(
          { _id: userLikeList[i] },
          projection
        );
      ...
      } else if (n + skip >= userLikeList.length) {
        has_more = false;
      }
